...
static int FLAGS_vtables=12;

static int FLAGS_freesize;
...
static const char* NVM_db = "/mnt/pmem0/";

namespace leveldb {

namespace {
leveldb::Env* g_env = nullptr;
leveldb::Thread_gc* bench_gc = nullptr;
...
class Stats {
 private:
  ...
  double seconds_;
  double seconds_load;
  double seconds_update1;
  double seconds_update2;
  int done_;
  int done_load;
  int done_update1;
  int done_update2;
  ...
  int64_t bytes_;
  int64_t bytes_load;
  int64_t bytes_update1;
  int64_t bytes_update2;
 ...
 public:
  ...
  void Merge(const Stats& other) {
    ...
    done_ += other.done_;
    done_load += other.done_load;
    done_update1 += other.done_update1;
    done_update2 += other.done_update2;
    bytes_ += other.bytes_;
    bytes_load += other.bytes_load;
    bytes_update1 += other.bytes_update1;
    bytes_update2 += other.bytes_update2;
    seconds_ += other.seconds_;
    seconds_load += other.seconds_load;
    seconds_update1 += other.seconds_update1;
    seconds_update2 += other.seconds_update2;
  ...
  void StopLoad() { seconds_load = (g_env->NowMicros() - start_) * 1e-6; }
  void Stop() {
  ...
  void Getdone_load() { done_load = done_; }
  void Getdone_update1() { done_update1 = done_ - done_load; }
  void Getdone_update2() { done_update2 = done_ - done_load - done_update1; }
  void AddBytes(int64_t n) { bytes_ += n; }
  void AddBytesLoad(int64_t n) { bytes_load += n; }
  void AddBytesUpdate1(int64_t n) { bytes_update1 += n; }
  void AddBytesUpdate2(int64_t n) { bytes_update2 += n; }
  void Report(const Slice& name) {
    // Pretend at least one op was done in case we are running a benchmark
    // that does not call FinishedSingleOp().
    if (done_ < 1) done_ = 1;
    if (name.ToString() == "UpdateWrite") {
      std::string extra;
      if (bytes_load > 0) {
        ...
        std::snprintf(rate, sizeof(rate), "%6.1f MB/s",
                      (bytes_load / 1048576.0) / elapsed);
        extra = rate;
      }
      AppendWithSpace(&extra, message_);
      std::fprintf(stdout, "Load阶段");
      std::fprintf(stdout, "%-12s : %11.3f micros/op;%s%s\n",
                   name.ToString().c_str(), seconds_ * 1e6 / done_load,
      ...
      std::fprintf(stdout, "%-12s : %11.3f micros/op;%s%s\n",
                   name.ToString().c_str(),
                   seconds_ * 1e6 / done_,  // done_/seconds_ / 1e3, KOPS
                   (extra.empty() ? "" : " "), extra.c_str());
...
class Benchmark {
 private:
  // PM_allocator* pm_alloc;
  std::shared_ptr<PM_allocator> pm_alloc=nullptr;
  Cache* cache_;
  const FilterPolicy* filter_policy_;
  DB* db_;
  int num_;
  uint64_t vtables_threshold;
  uint64_t free_size;
  int value_size_;
 ...
 public:
  Benchmark()
      : cache_(FLAGS_cache_size >= 0 ? NewLRUCache(FLAGS_cache_size) : nullptr),
        ...
        db_(nullptr),
        num_(FLAGS_num),
        vtables_threshold(FLAGS_vtables),
        free_size(FLAGS_freesize),
    ...
    if (!FLAGS_use_existing_db) {
      DestroyDB(FLAGS_db, NVM_db, Options());
    }
  }
  ...
  void Run() {
    ...
    while (benchmarks != nullptr) {
      ...
      if (name == Slice("fillseq")) {
        ...
        method = &Benchmark::WriteSeq;
      } else if (name == Slice("UpdateWrite")) {
        fresh_db = true;
        entries_per_batch_ = 1000;
        method = &Benchmark::UpdateWrite;
      } else if (name == Slice("fillrandom")) {
        fresh_db = true;
        method = &Benchmark::WriteRandom;
        // ComulateWA();
      } else if (name == Slice("overwrite2")) {
        fresh_db = false;
        method = &Benchmark::OverWrite2;
      } else if (name == Slice("overwrite1")) {
        fresh_db = false;
        method = &Benchmark::OverWrite1;
 ...
 private:
  ...
  void RunBenchmark(int n, Slice name,
    ...
    for (int i = 0; i < n; i++) {
      ...
      g_env->StartThread(ThreadBody, &arg[i]);  //开启线程
    }
  ...
  void ComulateWA()
  {
    double wa=db_->GetWritten()/((16 + FLAGS_value_size * FLAGS_compression_ratio) * num_);
    std::cout<<db_->GetWritten()<<std::endl;
    std::fprintf(stdout,"Write Amplification:%.1f\n",wa);
  ...
  void Open() {
    printf("benchmark！open");
    assert(db_ == nullptr);
    Options options;
    options.vtables_threshold=vtables_threshold;
    options.free_size=free_size;
    options.env = g_env;
    options.gc = bench_gc;
    ...
    Status s = DB::Open(options, FLAGS_db, NVM_db, &db_);
    if (!s.ok()) {
  ...
  //     delete db_;
  //     Open();
  //     thread->stats.FinishedSingleOp();//进行若干次Open操作
  //   }
  // }
  ...
  void DoWrite(ThreadState* thread, bool seq) {
    ...
    int64_t bytes = 0;
    KeyBuffer key;
    std::fprintf(stdout, "load numbers:%d\n", num_);
    for (int i = 0; i < num_; i += entries_per_batch_) {
      ...
      for (int j = 0; j < entries_per_batch_; j++) {
        ...
        key.Set(k);
        std::string ikey = key.slice().ToString();
        loadKeys[i] = ikey;
  ...
  void OverWrite1(ThreadState* thread) { DoWrite1(thread, false); }
  void DoWrite1(ThreadState* thread, bool seq) {
    // if (num_ != FLAGS_num) {
    //   char msg[100];
    //   std::snprintf(msg, sizeof(msg), "(%d ops)", num_);
    //   thread->stats.AddMessage(msg);
    // }
    num_ /= 2;
    std::fprintf(stdout, "update numbers:%d\n", num_);
    RandomGenerator gen;
    WriteBatch batch;
    Status s;
    int64_t bytes = 0;
  ...
  void OverWrite2(ThreadState* thread) { DoWrite2(thread, false); }
  ...
  void UpdateWrite(ThreadState* thread) {
    std::unordered_map<int, std::string> loadKeys;
    if (num_ != FLAGS_num) {
      char msg[100];
      std::snprintf(msg, sizeof(msg), "(%d ops)", num_);
    ...
    thread->stats.AddBytesLoad(bytes);
    thread->stats.Getdone_load();

    // update
    for (int round = 0; round < 2; round++) {
      bytes = 0;
      for (int i = 0; i < num_ / 2; i += entries_per_batch_) {
        batch.Clear();
        for (int j = 0; j < entries_per_batch_; j++) {
          std::string ikey = loadKeys.at(round * num_ / 2 + i + j);
          batch.Put(Slice(ikey), gen.Generate(value_size_));
          // printf("key: %s \n",key.slice().data());
          bytes += value_size_ + key.slice().size();
          thread->stats.FinishedSingleOp();
        }
      ...
      if (round == 0) {
        thread->stats.Getdone_update1();
        thread->stats.AddBytesUpdate1(bytes);
      } else {
        thread->stats.AddBytesUpdate2(bytes);
        thread->stats.Getdone_update2();
      }
    }
    // thread->stats.AddBytes(bytes);
  }
...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    }else if(sscanf(argv[i], "--vtables=%d%c", &n, &junk) == 1){
      FLAGS_vtables=n;
    }else if(sscanf(argv[i], "--freesize=%d%c", &n, &junk) == 1){
      FLAGS_freesize=n;
  ...
  leveldb::bench_gc = new leveldb::Thread_gc();

  // Choose a location for the test database if none given with --db=<path>
  if (FLAGS_db == nullptr) {
    leveldb::g_env->GetTestDirectory(&default_db_path);
    default_db_path += "/dbbench";
    FLAGS_db = default_db_path.c_str();  //数据库文件存放的位置
  }
