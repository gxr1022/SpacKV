...
class Repairer {
 public:
  Repairer(const std::string& dbname, const Options& options, PM_allocator *alloc_)
      : dbname_(dbname),
        env_(options.env),
 ...
 private:
  ...
  Status ConvertLogToTable(uint64_t log) {
    ...
    status = BuildTable(dbname_, env_, options_, table_cache_, iter, &meta,pm_alloc);
    delete iter;
    mem->Unref();
    mem = nullptr;
    if (status.ok()) {
  ...
  std::shared_ptr<PM_allocator> pm_alloc;
};
}  // namespace

Status RepairDB(const std::string& dbname, const Options& options, PM_allocator* alloc_) {
  Repairer repairer(dbname, options, alloc_);
  return repairer.Run();
}
